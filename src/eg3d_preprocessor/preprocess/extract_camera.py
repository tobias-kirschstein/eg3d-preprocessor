...
class CameraExtractor:
    ...
    def crop(self, img_pil, lm) -> np.ndarray:
        rescale_factor = 300
    ...
    def __extract(self, image_pil):
        ...
        return camera, image_cropped

    def extract(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        image_pil = Image.fromarray(image)  # prevent png exist channel error
        camera, image_cropped = self.__extract(image_pil)
